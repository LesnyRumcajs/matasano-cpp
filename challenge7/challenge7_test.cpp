...
TEST(Challenge7Test, AES_ECB_decrypt) {
  auto data = tools_file::bytesFromBase64EncodedFile("../challenge7/7.txt");
  std::string key_str{"YELLOW SUBMARINE"};
  std::vector<uint8_t> key{key_str.begin(), key_str.end()};
  auto result_vec = challenge7::decryptAES_ECB(data, key);

  std::string result{result_vec.begin(), result_vec.end()};
  std::string expected =
